...
$ourakea_featured_id = absint( get_theme_mod( 'post_dropdown_setting' ) );

if ( empty( $ourakea_featured_id ) ) {
	return;
}

$thumbnail = wp_get_attachment_image_src( get_post_thumbnail_id( $ourakea_featured_id ), 'full' );

	// Getting post by ID.
	$query = new WP_Query( 'p=' . $ourakea_featured_id );
